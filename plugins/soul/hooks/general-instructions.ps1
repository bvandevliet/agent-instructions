...
$contentPath = Join-Path $PSScriptRoot '..\instructions\general.instructions.md'
...
# Strip the YAML frontmatter fence (description/applyTo): that's VS Code
# instructions-file metadata, not instructional text for the model.
$content = ($content -replace '(?s)^---.*?---', '').TrimStart()
