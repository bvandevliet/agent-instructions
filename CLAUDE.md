...
Note there are two files named `CLAUDE.md` in this repo, with unrelated purposes: **this one** (repo root) is meta, guidance for editing the library itself, and `instructions/general/CLAUDE.md` is *content*, the general instructions this library ships to consumers of the plugin. Don't confuse edits to one for the other.
...
Neither content file carries a maintainer HTML comment anymore — that context now lives here, so both stay clean for their actual consumers. `instructions/general/CLAUDE.md`'s comment used to be stripped by `scripts/claude-general-instructions.ps1` before injection anyway, so moving it out cost nothing. `instructions/general/bvandevliet-general.instructions.md`'s comment was riskier to leave in place: whether Copilot strips HTML comments from `.instructions.md` files before the model sees them is unconfirmed, so a maintainer note living there could have been silently burning tokens in the actual instructions the model reads.

`instructions/general/CLAUDE.md` also currently doubles as the manual-copy source for anyone not using the plugin (e.g. pasting into `~/.claude/CLAUDE.md` directly) — a single fixed-name file, so no naming collision risk today. When `rules-via-plugin` (`anthropics/claude-code#21163`) ships: switch to it, drop the `SessionStart` hook, and rename this file with a unique prefix at that point (matching `bvandevliet-general.instructions.md`'s `bvandevliet-` convention). Once the plugin ships instructions straight into `~/.claude/rules/`, that folder becomes the same kind of shared, arbitrarily-named-file space as Copilot's `~/.copilot/instructions/` — the collision risk only becomes real at that point, not before.
